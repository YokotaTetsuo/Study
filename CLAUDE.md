...
4. Copilot の指摘を反映し、再 push してレビューを再依頼する。
   **新規の有効な指摘が付かなくなるまで** 繰り返す（再掲・誤検知は根拠を示す）。
   ただしドキュメントのみの PR は後述「ドキュメントのみの PR の収束方針」
   に従い、反復要件を緩和する。
...
Copilot レビュー待ちは時間がかかるため、**レビュー待ちの間は別の独立した
PR に着手してよい**（依存関係のある PR は依存先のマージ後）。レビューは
到着した PR から順に指摘対応する（1 本ずつ直列に収束させる必要はない）。
各 PR は独立ブランチで管理し、main からの最新で作業する。

### ドキュメントのみの PR の収束方針

`CLAUDE.md` / `docs/` などドキュメントだけの PR は、**事実誤り・手順の
不整合（再現性に関わるもの）のみブロッキング**として対応する。語彙・
言い回し等の軽微な Copilot 指摘は収束を遅らせないため、根拠を一言添えて
**CI 緑かつ実害のある指摘が無くなった時点でマージしてよい**（実装 PR の
ような「新規指摘ゼロまで反復」は要求しない）。

...
Google の Small / Medium / Large 分類を採用し、ファイル名サフィックスで物理分離する。現時点では Small / Medium を PR push 毎に実行する (Large / E2E は `docs/TASKS.md` の Phase 6.3〈任意〉で追加予定。詳細は 7 章を参照)。
...
現時点では **Small / Medium を PR push 毎に実行** する (Large / E2E は `docs/TASKS.md` の Phase 6.3〈任意〉で追加予定)。

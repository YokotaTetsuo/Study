...
describe('DocumentId', () => {
  it('should accept a valid ULID', () => {
    expect(new DocumentId(VALID).value).toBe(VALID);
  });

  it('should compare by value', () => {
    expect(new DocumentId(VALID).equals(new DocumentId(VALID))).toBe(true);
    expect(
      new DocumentId(VALID).equals(
        new DocumentId('01HQ8ZK9PRSTVWXYZ23456789B'),
      ),
    ).toBe(false);
  });
  ...
  it.each([
    ...
    expect(() => new DocumentId(input)).toThrow(ValidationError);
  });
});

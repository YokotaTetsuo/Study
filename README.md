...
`seed` は冪等かつ回復可能: 各エンティティの存在を都度確認し、未作成分
だけを作る（途中失敗しても再実行で続きから完了する）。**この
スクリプトが新規作成したアカウントのみ**パスワードが `password1234`。
既に存在するアカウントは触らず、従来のパスワードのまま（実行ログに
新規作成/再利用の内訳が出る）。クリーンな DB に対しては以下 4 名と
承認ポリシー（owner / approver が承認可）が用意される:
